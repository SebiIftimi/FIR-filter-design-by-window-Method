function [Delta_pr, Delta_sr] = evaluareaFiltruFIR(h, omega_p, omega_s)
    ...
    H = abs(H);  % Calculating the magnitude of the frequency response

    % Extracting the passband and stopband
    % Passband: [0, omega_p]
    ...
    Delta_pr = max(abs(1 - H_p)) * 100;  % Multiply by 100 to express in percent

    % Maximum deviation in the stopband
    Delta_sr = max(H_s) * 100;  % Multiply by 100 to express in percent
end
